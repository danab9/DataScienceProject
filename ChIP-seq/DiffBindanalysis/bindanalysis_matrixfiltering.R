...
#read in file that got created by unionbedg (big matrix)
peak_df <- read.table("H3K122ac_mat.txt")
...
#check that in at least 20 of the 27 patients values are present so there is not too much missing values
count <- rowSums(peak_df[,4:30] != 0) > 20
length(count[count==TRUE])

#save the selected rows from previous analysis as new matrix
mat_23 <- peak_df[which(rowSums(peak_df[,4:30] != 0)>20),]
...
#show distribution of ranges in genome - exon/intron and genomic element distribution in general
...
while(count < dim(mat_TSS_filtered)[1]){
  ...
  while(sum((mat_TSS_filtered[count,4:30] == mat_TSS_filtered[count+size,4:30]) == TRUE)>15 && count+size < dim(mat_TSS_filtered)[1]){ #21
    size=size+1
  }
...
write.table(mat_TSS_filtered[resvec,], file="H3K122ac_mat.txt", row.names = FALSE, col.names = FALSE, sep="\t")
