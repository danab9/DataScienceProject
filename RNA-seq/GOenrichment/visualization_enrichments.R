...
treeplot(pairwise_termsim(up_MFs), showCategory = 20, hclust_method = "average") + ggtitle( "downregulated Cellular Component GO Terms")
...
ids<-bitr(downregulated$genes, fromType = "ALIAS", toType = "ENTREZID", OrgDb= org.Hs.eg.db)
dedup_ids = ids[!duplicated(ids[c("ALIAS")]),]

up.ids<- bitr(upregulated$genes, fromType = "ALIAS", toType = "ENTREZID", OrgDb= org.Hs.eg.db)
up.dedup_ids = up.ids[!duplicated(up.ids[c("ALIAS")]),]


df2 = right_join(downregulated, dedup_ids, by= c("genes" = "ALIAS"))
down.gene.list.ID <- (df2$log2FoldChange)
# Name vector with ENTREZ ids
names(down.gene.list.ID) <- as.character(df2$ENTREZID)
# sort the list in decreasing order (required for clusterProfiler)
down.gene.list.ID = sort(down.gene.list.ID, decreasing = TRUE)


df3 = right_join(upregulated, up.dedup_ids, by= c("genes" = "ALIAS"))
up.gene.list.ID <- (df2$log2FoldChange)
# Name vector with ENTREZ ids
names(up.gene.list.ID) <- as.character(df3$ENTREZID)
# sort the list in decreasing order (required for clusterProfiler)
up.gene.list.ID = sort(up.gene.list.ID, decreasing = TRUE)




## not working 
up_kk <- gseKEGG(geneList = down.gene.list.ID,
               organism     = "hsa",
               nPermSimple   = 10000,
               minGSSize    = 1,
               maxGSSize    = 800,
...
## Reactome Pathways instead of KEGG
library(ReactomePA)
down.pathways <- enrichPathway(names(down.gene.list.ID), organism = "human", pvalueCutoff=0.05,pAdjustMethod = "BH")
up.pathways <-  enrichPathway(names(up.gene.list.ID), organism = "human", pvalueCutoff=0.05,pAdjustMethod = "BH")
